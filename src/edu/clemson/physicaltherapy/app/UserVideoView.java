...
public class UserVideoView extends VideoViewActivity
{
	 ...
	 * @brief Inflate the menu; this adds items to the action bar if it is
	 *        present.
	 * @param menu
	 *            Meny to be created.
	 * @return true
	...
	private void exportVideo()
	{
		...
		try
		{
			...
			String message = "Exercise: " + exercise.getName() + "\n"
					+ "Exercise instructions: " + exercise.getInstructions()
					+ "\n" + "Time Exercise Performed: "
					+ exerciseLog.getCreateTime() + "\n" + "Notes: "
					+ exerciseLog.getCreateTime() + "\n" + "Audio Notes: "
					+ hasAudioNotes + "\n";
			...
			{
				// Video is between the last '/' and the '.' for the file
				// extension.
				...
				File annotationsFile = new File(this.getExternalFilesDir("export").getCanonicalPath(), "Annotations-"
						+ video_name + ".csv");
	...
	private void createCSVForAnnotations(File outputFile,
			List<ExerciseLogAnnotation> annotationList)
	{
		...
		try
		{
			...
			for (int i = 0; i < annotationList.size(); i++)
			{
				...
				writer.append(hours + ":" + minutes + ":" + seconds + "."
						+ mSeconds);
				writer.append("\",\"");
	...
	public void updateAnnotation(int time, String annotation, int interval)
	{
		...
		else
		{
			...
			// Do not update time when updating annotation. Prevents
			// "annotation" creep.
