 ...
 * 
 * @brief This abstract class provides database connectivity (internal and
 *        external) to activities in the app.
 * @brief Activities that want to connect to the database should inherit from
 *        this class.
 */
...
public abstract class DatabaseActivity extends DisplayActivity
{
	 ...
	 * @brief This is the SQLite handler. Every app that connects to the
	 *        internal database will need a database handler to access it
	 */
	protected DatabaseHandler		dbSQLite			= new DatabaseHandler(this);

	/**
	 * @var private DatabaseObject myDatabaseObject
	 * @brief Global variable pointing to a database object. This is the object
	 *        that is used for querying the external database.
	 */
	 ...
	 * @brief This is an async task that adds the database object stored in
	 *        myDatabaseObject to the external database. This task is started by
	 *        the this.addToExternalDatabase() method.
	 * 
	 */
	...
	class AddDatabaseObject extends AsyncTask<String, String, String>
	{
		...
		protected String doInBackground(String... args)
		{
			...
			{
				System.err.print(params.get(index).getName() + "="
			...
			try
			{
				...
				JSONObject json = jsonParser.makeHttpRequest(myDatabaseObject.getAddUrl(), "POST", params);
				...
				if (success == 1)
				{
					// add returns the object created. This is to let MySQL take
					// care of the autogeneration of indices.
					JSONArray productObj = json.getJSONArray(myDatabaseObject.getTableName()); // JSON
																								// Array
	 ...
	 * @brief This is an async task that updates the database object stored in
	 *        myDatabaseObject in the external database. This task is started by
	 *        the this.updateInExternalDatabase() method.
	 * 
	 * 
	 */
	class UpdateDatabaseObject extends AsyncTask<String, String, String>
	{
		...
		protected String doInBackground(String... args)
		{
			...
			try
			{
				...
				System.err.println("Updating via "
						+ myDatabaseObject.getUpdateUrl());
				JSONObject json = jsonParser.makeHttpRequest(myDatabaseObject.getUpdateUrl(), "POST", params);
	 ...
	 * @brief This is an async task that deletes the database object stored in
	 *        myDatabaseObject from the external database. This task is started
	 *        by the this.DeleteInExternalDatabase() method.
	 * 
	 * 
	 */
	...
	class DeleteDatabaseObject extends AsyncTask<String, String, String>
	{
		...
		protected String doInBackground(String... args)
		{
			...
			try
			{
				...
				JSONObject json = jsonParser.makeHttpRequest(myDatabaseObject.getDeleteUrl(), "POST", params);
	 ...
	 * @brief This is an async task that deletes all database objects for a
	 *        given table. This does NOT use myDatabaseObject, but takes a table
	 *        name as a parameter to doInBackground. This task is started by the
	 *        this.DeleteAllRecordsFromExternalDatabase() method.
	 * 
	 * 
	 */
	...
	class DeleteAllDatabaseObjects extends AsyncTask<String, String, String>
	{
		...
		protected String doInBackground(String... args)
		{
			...
			try
			{
				...
				System.err.println("deleteAllUrl="
						+ DatabaseObject.getDeleteAllUrl());
				// getting product details by making HTTP request
				JSONObject json = jsonParser.makeHttpRequest(DatabaseObject.getDeleteAllUrl(), "POST", params);
	 ...
	 * @brief This is an async task that gets an object from the external
	 *        database from the id of the object stored in myDatabaseObject and
	 *        stores it in the databaseObjectList. myDatabaseObjectList should
	 *        have one object in it.
	 * 
	 */
	class GetDatabaseObjectById extends AsyncTask<String, String, String>
	{
		...
		protected String doInBackground(String... args)
		{
			...
			try
			{
				...
				JSONObject json = jsonParser.makeHttpRequest(myDatabaseObject.getByIdUrl(), "GET", params);
	 ...
	 * @brief This is an async task that gets all objects from the external
	 *        database for a given table and stores them in the
	 *        databaseObjectList. The table information will be contained in
	 *        myDatabaseObject. This can be a "dummy" object of the appropriate
	 *        class with no data in it.
	 * 
	 */
		...
		protected String doInBackground(String... args)
		{
			...
			try
			{
				...
				if (success == 1)
				{
					...
					System.err.println("Found " + products.length()
							+ " objects for " + myDatabaseObject.getTableName());
	 ...
	 * @brief Adds a databaseObject to the list from a JSON Object. The type of
	 *        database object should match the type of myDatabaseObject.
	 * @param c
	...
	private void addToListFromJSON(JSONObject c) throws JSONException
	{
		...
		System.err.println("Adding new " + newObj.getTableName()
				+ " object to list");
	...
	/**
	 * @fn protected void updateInExternalandInternalDatabase(DatabaseObject
	 *     dbo)
	 * @brief Update the given DatabaseObject dbo in external and internal
	 *        database.
	 * @param dbo
	 */
	 ...
	 * @brief Add the given DatabaseObject dbo to the external and internal
	 *        database.
	 * @param dbo
	 */
	...
	protected void addToExternalAndInternalDatabase(DatabaseObject dbo)
	{
		...
		try
		{
			...
			{
				// The global myDatabaseObject does not contain the newly index
				// or any MySQL formatting adjustments.
	...
	/**
	 * @fn protected void deleteFromExternalAndInternalDatabase(DatabaseObject
	 *     dbo)
	 * @brief Delete the given DatabaseObject dbo from the external and internal
	 *        database.
	 * @param dbo
	 */
	...
	/**
	 * @fn protected void deleteAllRecordsFromExternalAndInternalTable(String
	 *     tableName)
	 * @brief delete all the records from the internal and external database for
	 *        table tableName.
	 * @param tableName
	 */
	 ...
	 * @brief This method rebuilds the internal table from the external. This
	 *        method should do the following 1. Get all objects from the
	 *        external database 2. Add new objects that have been created in the
	 *        external database. 3. Update existing objects in internal database
	 *        that have been changed in external database. 4. Delete objects
	 *        that have been removed from external database. 5. Maintain data
	 *        integrity.
	 * 
	 *        Note: External and internal autogenerated indices will not match
	 *        if both are autogenerated. External and internal data records
	 *        should either have matching ids or a way to map external and
	 *        internal ids.
	 * @param dbo
	 * 
	 */
		...
		try
		{
			...
			for (int index = 0; index < databaseObjectList.size(); index++)
			{
				System.err.println("Adding id"
						+ databaseObjectList.get(index).getId() + " for table "
						+ databaseObjectList.get(index).getTableName());
				// TODO: Intelligently add/update/delete object from the
				// database.
	 ...
	 *        from the external database and storing it in the internal
	 *        database.
	 */
